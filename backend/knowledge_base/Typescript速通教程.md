...
- JavaScript 当年诞⽣时的定位是**浏览器脚本语⾔**,⽤于在⽹⻚中嵌⼊简单的逻辑,且代码 量很少。
...
### 2. ⾃动化编译
- 第⼀步:创建 TypeScript 编译控制⽂件：`tsc --init`（会生成tsconfig.js文件）
...
- 类型声明，作为变量或函数形参
  ...
  ```ts
  let a: string  // 变量a只能存储字符串（注意：string小写）
...
3. 两个用于自定义类型的方式： ① type ② interface
   ...
   > 注意点
   > 在 TypeScript 中进⾏类型声明时,通常都是⽤**小写**的 number 、 string 、 boolean
...
- 弊端：⼏乎**不**⽤ never 去直接**限制变量**,因为没有意义,因为所有的赋值都会有警告

- 用处：

  1. never 一般是 TypeScript 主动推断出来的，例如：
  ...
  2. never ⽤于限制函数的返回值（返回值是never，就意味着函数不能结束，或正常结束）
...
1. void 通常⽤于函数返回值声明
   ...
   即: 函数不返回任何值, 调⽤者也不应依赖其返回值进⾏任何操作!

   ```ts
   function logMessage(msg:string):void{
   	console.log(msg)
       //return/  /return undefined;
       //注意：以上三种表示方法都对，因为它的返回值是undefined
   }
   logMessage('你好')
   ```

2. 注意： undefined 和 void 在限制函数返回值时都返回undefined，但返回值为void的函数，调⽤者也不应依赖其返回值进⾏任何操作!
...
- **object**(⼩写)
  ...
  object (⼩写)的含义是:所有**非原始**类型,可存储:对象、函数、数组等,   

  > 注意：number、boolean、string、null、undefined是原始类型
...
1. **声明对象类型**
   ...
   - 可选属性
     ...
     > 注意：声明对象时，属性之间可以用逗号、分号、换行隔开
     >
     > ​           命值对象时，有问号的属性可选可不选，没有声明属性名不能直接命值
   ...
   - 索引签名
     ...
     > 索引签名,完全可以不⽤key这个单词,换成其他的也可以(另一种常用写法：index
...
2. **声明函数类型**
   ...
   - JavaScript 中的 => 是⼀种定义函数的语法，特指箭头函数。 
...
- 元组 (Tuple) 是⼀种**特殊的数组**类型，可以存储固定数量的元素,并且每个元素的类型是已知的且可以不同。
...
type 可以为任意类型**创建别名**,让代码更简洁、可读性更强,同时能更⽅便地进⾏类型复⽤和 扩展。
...
- **联合类型**
  ...
  ```ts
  type Status = number | string;    //Stutus只能是数字或字符串
  type Gender = '男' | '女';        //Gender只能是男或女
  ```
...
```ts
// LogFunc的类型是一个函数，该函数不接受任何参数，并且其返回值是void类型的
type LogFunc = () => void  
...
为什么会这样?
是**为了确保如下代码成立**,我们知道 Array.prototype.push 的返回值是⼀个数字, ⽽ Array.prototype.forEach ⽅法期望其回调的返回类型是 void 。
```ts
const src = [1, 2, 3];
const dst = [0];
...
- interface 是⼀种**定义结构的方式**，主要作⽤是为：类、对象、函数等规定一种契约，
...
- 相同点: interface 和 type 都可以用于定义对象结构,在定义对象结构时两者可以 互换。 
...
泛型允许我们在**定义函数、类或接口**时,使用类型参数来**表示未确定的类型**,这些参数在具体使⽤时,才被指定具体的类型,泛型能让同⼀段代码适⽤于多种类型,同时仍然保持类型的安全性。
...
- 基本用法：
  ...
  代码中 `<T>` 就是泛型,(不⼀定非叫 T ),设置泛型后即可在函数中使⽤ T 来表 示该类型
...
作用是为现有的JavaScript 代码提供类型信息，使得 TypeScript 能够在使用这些 JavaScript 库或模块时进行类型检查和提示。
